...
let S = {
  ...
  scheduled: [], scheduledLoading: true,
  clientSearch: '',
  clientFilter: 'all',
  modal: null,
  detail: null, detailLoading: false, detailTab: 'info',
};
...
async function fetchScheduled() {
  ...
  try { set({ scheduled: await req('GET','/scheduled/') || [], scheduledLoading: false }); }
  catch { set({ scheduledLoading: false }); }
}
          ...
          <tbody>${all.map(a => `<tr>
            ...
            <td>${a.status==='pending' ? `
              <div class="flex aic gap1">
                <button class="btn btn-green btn-sm" onclick="openApproval(${a.id})">Review</button>
                <button class="btn btn-red btn-sm" onclick="quickReject(${a.id})">✕</button>
              </div>` : `<span class="muted" style="font-size:11px">${a.reviewed_at ? ago(a.reviewed_at) : '—'}</span>`}
            </td>
          </tr>`).join('')}</tbody>
        </table></div>`
  }
...
function renderModal() {
  ...
  if (m.type === 'detail') {
    ...
    <div class="modal modal-lg">
      ...
      <div class="modal-body">
        ${loading ? '<div class="loading"><span class="spin"></span></div>' : !c ? '<div class="loading muted">Not found</div>' : `
        <div class="tabs">
        ...
        <div class="mb4">
          ${c.children.map(ch => `<div class="flex aic gap2 mb2">
            <span>👶</span>
        ...
        ` : `
        <div class="msg-list">
          ${!(c.recent_messages && c.recent_messages.length) ? `<div class="empty"><p>No messages yet</p></div>` :
          ...
          <div class="f-group">
            ...
            <select class="f-input" id="j-phase">
              ${['entry','package','booking','prep','delivery','feedback'].map(p =>
                `<option value="${p}" ${c.phase===p?'selected':''}>${p}</option>`).join('')}
            </select>
          </div>
          <div class="f-group">
