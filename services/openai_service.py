...
def build_system_prompt(
    ...
    return (
        ...
        f"- PACKAGE PRESENTATION FORMAT (use exactly this structure):\n"
        f"  *Starter* — 50,000 RWF\n"
        f"  1h session, 8 edited photos + all unedited\n"
        f"\n"
        f"  *Silver* — 70,000 RWF\n"
        f"  1.5h session, 12 edited photos + all unedited\n"
        f"\n"
        f"  *Gold* — 100,000 RWF\n"
        f"  1.5h session, 15 edited photos + all unedited\n"
        f"\n"
        f"  Which one feels right for you?\n"
