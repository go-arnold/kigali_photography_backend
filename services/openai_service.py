...
def build_system_prompt(
    ...
    return (
        ...
        f"- CONVERSATION STATE: Journey is '{journey_phase}' / '{journey_step}'.\n"
        f"- If journey_phase is 'entry' AND there are NO assistant messages in history: send the intro greeting.\n"
        f"- If there are ANY assistant messages already in history: NEVER send the intro. Continue the discovery from where it stopped.\n"
        f"- The conversation history below shows EXACTLY where you are — read it before responding.\n"
        f"- Your next response must be the LOGICAL NEXT STEP based on the last message exchanged.\n"
        f"- If there are already messages in the conversation history: NEVER send the intro again. Continue naturally from where the conversation left off.\n"
        f"- If client says hello again after a gap: respond with 'Hello again! How can I help you today?' NOT the full intro.\n"
        ...
        f"- NEVER repeat the introduction message if conversation history already exists.\n"
        f"- NEVER send the intro if journey_step is not 'entry' or if previous messages exist.\n"
